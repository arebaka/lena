...
module.exports = async (mess, creatorId, action, factor, fullFactor=false, strictCase=false, autoDelete=0) => {
    let trigger;

    if (mess.text) {
        trigger = await db.addTextTrigger(
            mess.chat.id, creatorId, action, factor, fullFactor, strictCase, autoDelete, mess.text
        );
    }
    else if (mess.animation) {
        trigger = await db.addFileTrigger(
            mess.chat.id, creatorId, action, factor, fullFactor, strictCase, autoDelete,
        ...
        trigger = await db.addDiceTrigger(
            mess.chat.id, creatorId, action, factor, fullFactor, strictCase, autoDelete, mess.dice.emoji
        );
    }
